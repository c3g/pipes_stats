...
export default class PipesLineChart extends React.Component {
  onMouseEnter = name => {
    if (this.props.activePipeline !== name) this.props.onMouseEnter && this.props.onMouseEnter(name)
  }

  onMouseLeave = name => {
    if (this.props.activePipeline === name) this.props.onMouseLeave && this.props.onMouseLeave(name)
  }
  ...
  render() {
    ...
    return (
      <AutoSizer disableHeight>
        {({ width }) => (
          <LineChart width={width} height={400} data={data} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
            ...
            {keys.map((key, i) => {
              ...
              return (
                <Line
                  ...
                  dataKey={key}
                  stroke={
                    activePipeline === undefined ? colors[key] : isActive ? colors[key] : hexToRGBA(colors[key], 0.3)
                  }

...
class PipesPieChart extends React.Component {
  ...
  onMouseMove = (data, index) => {
    if (this.props.activePipeline !== data.name) this.props.onMouseEnter && this.props.onMouseEnter(data.name)
  }
  ...
  onDocumentMouseMove = ev => {
    ...
    if (props.activePipeline && !containers.some(c => c.contains(target)) && !className.includes('recharts')) {
  ...
  componentDidMount() {
    ...
    if (this.container) containers.push(this.container)
  }
  ...
  render() {
    ...
    if (this.activePipeline != activePipeline) this.activePipeline = activePipeline

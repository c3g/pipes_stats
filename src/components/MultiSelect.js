...
class MultiSelect extends Component {
  ...
  render() {
    ...
    const items = Object.keys(values).map(value => (
      <li>
        <a key={value} href="#" className="MultiSelect-item" onClick={() => onChange(value, !values[value])}>
          <Icon name={values[value] ? 'check-square-o' : 'square-o'} />&nbsp;
          {value}
        </a>
      </li>
    ))
    ...
    return (
      <div ref="container" className={cx('MultiSelect dropdown', { open: visible })}>
        <button
          ref="button"
          type="button"
          ...
          disabled={loading}
        >
          <span>{checkedValues.join(', ') || (loading ? 'Loading...' : 'No value selected')}</span>
        </button>
        <Icon className="MultiSelect-icon" name={loading ? 'spinner fa-spin' : 'chevron-down'} />
        <ul ref="list" className="MultiSelect-list dropdown-menu">
          {items.length > 0 && (
            <li>
              <a href="#" className="MultiSelect-item" onClick={() => this.props.onChangeAll(!allSelected)}>
